...
module.exports = (opts) => {
  ...
  let configureRouter = (itemData, bundleMiddleware, pieProcessors) => {
    let router = express.Router();
    const lessMiddleware = require('less-middleware');

    logger.info('opts.root:', opts.root);
    router.use(bundleMiddleware);
  ...
  return npmDir.freshInstall(pies)
    .then(() => bundler.createEntryPoint(opts.root, pies))
    .then((entryPoint) => bundler.createMiddleware(opts.root, pies, entryPoint))
    .then((middleware) => {
      let p = readPieProcessors(pies, path.join(opts.root, 'node_modules'))
        .then((processors) => {
          return Promise.resolve({middleware: middleware, processors: processors});
        });
      return p
    })
    .then((data) => configureRouter(itemData, data.middleware, data.processors))
};