module.exports = function Container() {
  ...
  const EmptyEventService = require('./services/events/empty');


  let logger = logFactory.fileLogger(__filename);

  function startServer(port, root, itemData, resolutionReport, pieProcessors, logRequests) {
    var App = require('./server');
    var app = new App(root, itemData, resolutionReport, new Processor(pieProcessors), new EmptyEventService(), logRequests);
  ...
  this.run = function (opts) {
    ...
    if (bowerDir.isInstalled() && !opts.fullInstall) {
      ...
      readPieProcessors(opts.pies, bowerDir.components())
        .then((pieProcessors) => {
          startServer(opts.port, opts.root, itemData, bowerDir.resolutionReport(), pieProcessors, opts.logRequests);
        })
        .catch((err) => {
      ...
      bowerDir.cleanLocalInstallLink(_.extend({}, opts.pies, extraComponents), opts.resolutions)
        .then((resolutionReport) => {
          ...
          readPieProcessors(opts.pies, bowerDir.components())
            .then((pieProcessors) => {
              startServer(opts.port, opts.root, itemData, resolutionReport, pieProcessors, opts.logRequests);
            });
        })
        .catch((e) => {
