module.exports = function Container() {
  ...
  const eventHandlerBuilder = require('./services/events/builder');

  let logger = logFactory.fileLogger(__filename);

  function startServer(port, root, itemData, resolutionReport, pieProcessors, eventHandler, logRequests) {
    var App = require('./server');

    let dispatchEvent = (event) => {
      logger.debug('dispatchEvent for Processor, event: ', event);
      eventHandler.handle(event, 'some-url', 'serve-pie-demo');
    }

    let processor = new Processor(pieProcessors, dispatchEvent);
    var app = new App(root, itemData, resolutionReport, processor, eventHandler, logRequests);
  ...
  this.run = function (opts) {
    ...
    let eventHandler = eventHandlerBuilder(opts.eventHandlerPath);
