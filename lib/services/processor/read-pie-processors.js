...
module.exports = (pieKeys, componentsDir) => {
  ...
  logger.info('read pie processors for: ', _.keys(pieKeys), 'in dir: ', componentsDir);
  return new Promise((resolve, reject) => {
    let processors = _(pieKeys).keys().map((k) => {
      try {
        
        let loadProcessingModule = () => {
          let dir = path.join(componentsDir, k);
        ...
        let bowerPath = path.join(componentsDir, k, 'package.json');
        ...
        let script = new vm.Script(processingModule.src, {
          filename: processingModule.path
        });
