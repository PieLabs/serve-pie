...
function BowerDir(dir) {
  ...
  logger.info('bower version:', bower.version);
  ...
  this.clean = () => {
    ...
    let paths = [this.components(), dotInstalled, path.join(dir, 'bower.json')];
      ...
      bowerLogger
        ...
        .on('end', (linkResult) => {
          ...
          if (linkResult.src !== desiredPath) {
            ...
            bower.commands.link(name, null, {cwd: dir})
  ...
  this.localInstall = (dependencies, resolutions) => {
    ...
    logger.debug(`[install] ${JSON.stringify(dependencies)}`);
