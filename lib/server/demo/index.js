...
module.exports = (opts) => {
  ...
  let inlinePies = require('./read-inline-pies')(opts.pies);
  let itemPies = require('../../read-item-dependencies')(opts.root);
  let resolutions = (fs.readJsonSync(opts.root + '/dependencies.json', { throws: false }) || {}).resolutions;
  
  logger.silly('itemPies', itemPies, 'inlinePies:', inlinePies);
  
  let pies = _.extend({}, itemPies, inlinePies);
