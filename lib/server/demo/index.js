...
module.exports = (opts) => {
  ...
  let runNpmInstall = (pies) => {
    if(npmDir.isInstalled()){
      return Promise.resolve();
    } else {
      return npmDir.install(pies);
    }
  };
  ...
  /*let runBowerInstall = () => {
    if (bowerDir.isInstalled() && !opts.fullInstall) {
      ...
      extraComponents['pie-env-editor'] = 'git@github.com:PieLabs/pie-env-editor.git';
  ...
  return runNpmInstall(npmDir)
    .then(() => {
      return readPieProcessors(pies, path.join(opts.root, 'node_modules'))
        .then((processors) => {
          return configureRouter(itemData, {}, processors);
        });
    });
};