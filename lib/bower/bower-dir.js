...
function BowerDir(dir) {
      ...
      bowerLogger
        ...
        .on('end', (linkResult) => {
          ...
          if (linkResult.src !== desiredPath) {
            reject(new Error(`Bower linked to ${linkResult.src} but we wanted a link to: ${desiredPath}`));
          } else {
  ...
  this.localInstall = (dependencies, resolutions) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      let bowerLogger = bower.commands.install([], { save: false }, {});
